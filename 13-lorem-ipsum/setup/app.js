// lorem text
/* B1: get eles (form, amout, result)
B2: submit => 
---prevent default
-- get value of amount
---check constraints
++if len < 0 || len >9 || isNaN
++++ random text with random value
++else
++++get values use map function from arr
++++get in result*/
const text = [
...
form.addEventListener("submit", (e) => {
  ...
  if (isNaN(value) || value <= 0 || value > text.length) {
    ...
    const textTemp = text.slice(0, value);
    const htmlLorem = textTemp
      .map((text) => {
        return `<p class="result">${text}</p>`;
      })
      .join("");
    result.innerHTML = htmlLorem;
  }
});
